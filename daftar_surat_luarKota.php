...
?>
  ...
  <head>
    ...
    <title>DAFTAR SURAT KET LUAR KOTA</title>
    ...
    <section class="content">
      ...
      <div class="card-body">
        <div class="row">
          ...
          <h3 style="text-align:center">DAFTAR SURAT KETERANGAN KELUAR KOTA</h3>
            <table id="daftarrekom" class="table table-sm table-bordered table-hover">
              ...
              <tbody>
                <?php
                $ambildata  = (mysqli_query($con,"SELECT * FROM surat_ket_luarkota WHERE id_surat"));
                while($data=mysqli_fetch_array($ambildata)){
                  ...
                  <tr>
                    ...
                    <td><center>
                      <?php
                      $query  = "SELECT * FROM surat_ket_luarkota WHERE id_surat";
                      $run    = mysqli_query($con,$query);
                      ?>
                      <a href="surat_luarKota.php?id_surat=<?=$data['id_surat']?>"><i class="fas fa-file-alt"></i> View</button></a>
                      </center>
                      </td>
                      <td>
                      <center>
                      ...
                      <td>  
                      <center>
                        <form action="verif_surat_luarKota.php?id_surat=<?php echo $data['id_surat']; ?>" method="POST">
                          <input type="hidden" name="appid" value="<?php echo $data['id_surat']; ?>">
                          <input type="submit" class="btn btn-sm btn btn-success" data-mdb-ripple-color="dark" name="verifikasi" value="VERIF">
                        </form><hr>
                        <form action="tolak_surat_luarKota.php?id_surat=<?php echo $data['id_surat']; ?>" method="POST">
                          <input type="hidden" name="appid" value="<?php echo $data['id_surat']; ?>">
                      ...
                      <td>
                        <center> 
                          <a href="keterangan_luarKota.php?id_surat=<?=$data['id_surat']?>"><button class="btn btn-sm btn-info">Tambah Keterangan</button></a>
                        </center>
                      </td>
