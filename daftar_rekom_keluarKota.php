...
if ($data_user['level']!="admin" && $data_user['level']!="tamu") {
  ...
  <head>
    ...
    <title>DAFTAR REKOM KELUAR KOTA</title>
    ...
    <section class="content">
      ...
      <div class="card-body">
        <div class="row">
          ...
          <h3 style="text-align:center">DAFTAR REKOMENDASI KELUAR KOTA</h3>
            <table id="daftarrekom" class="table table-bordered table-hover">
              ...
              <tbody>
                <?php
                $ambildata  = (mysqli_query($con,"SELECT * FROM lampiran_luarkota JOIN integritas_luar_kota ON integritas_luar_kota.id_integritas=lampiran_luarkota.id_integritas"));
                while($data=mysqli_fetch_array($ambildata)){
                  ?>
                  <tr>
                    <td><left><?=$data['inp_nama']?></left></td>
                    <td><left><?=$data['jenis_rekomendasi']?></left></td>
                    <td><center>
                      <a href="detail_lamp_luarkota.php?id_luarKota=<?=$data['id_luarKota']?>"><i class="fas fa-file-pdf"></i> Lampiran</button></a>
                      </center>
                      </td>
                      <td>
                      <center>
                      ...
                      <td>
                        <center>
                        <form action="verif_kel_kota.php?id_luarKota=<?php echo $data['id_luarKota']; ?>" method="POST">
                          <input type="hidden" name="appid" value="<?php echo $data['id_luarKota']; ?>">
                          ...
                          <a href="saran_kel_kota.php?id_luarKota=<?=$data['id_luarKota']?>"><button type="button" class="btn btn-sm btn-outline-primary btn-rounded" data-mdb-ripple-color="dark">TULIS SARAN</button></a>
                        </form>
                    </center></td>
                    </tr>
                    <?php
