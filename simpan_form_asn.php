...
$query = "INSERT INTO form_pernyataan_asn 
(inp_nama,id_lampiran,inp_tempat_lahir,inp_jk,inp_dokter,inp_lulusan,inp_no_str,inp_tempatKerja,inp_alamat,inp_tlp,inp_anggota,inp_npa,sarana_kesehatan,alamat,tlp,kecamatan,kabupaten,sarana_kesehatan_2,alamat_2,tlp_2,kecamatan_2,kabupaten_2) 
VALUES
('$inp_nama','$id_lampiran','$inp_tempat_lahir','$inp_jk','$inp_dokter','$inp_lulusan','$inp_no_str','$inp_tempatKerja','$inp_alamat','$inp_tlp','$inp_anggota','$inp_npa','$sarana_kesehatan','$alamat','$tlp','$kecamatan','$kabupaten','$sarana_kesehatan_2','$alamat_2','$tlp_2','$kecamatan_2','$kabupaten_2')";
...
if($con->query($query)) {
  ...
  //redirect ke halaman index.php
   $ambil_id  = (mysqli_query($con,"SELECT * FROM form_pernyataan_asn WHERE inp_nama='$inp_nama'"));
  while($data=mysqli_fetch_array($ambil_id)){
  $id_lampiran = $data['id_lampiran'];
  header("location: pernyataan_asn.php?id=".$id_lampiran."&inp_nama=".$inp_nama);
}
